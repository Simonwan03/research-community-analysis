"""Small Semantic Scholar client with retry, throttling, and disk cache."""
...
class SemanticScholarClient:
    """Thin wrapper around a few Semantic Scholar endpoints."""
    ...
    def __init__(self, config: ApiConfig, cache_dir: Path | None = None, use_cache: bool = True):
        ...
        self.api_key = self._normalize_api_key(os.getenv(config.api_key_env_var, ""))
    ...
    def search_papers(self, query: str, year: int | None = None, limit: int | None = None) -> list[dict[str, Any]]:
        params: dict[str, Any] = {
            ...
            "fields": "paperId,title,year,authors,externalIds,venue,url,citationCount,referenceCount",
        }
        if year is not None:
            params["year"] = str(year)
        payload = self._request_json("GET", "/paper/search", params=params)
        return list(payload.get("data") or [])

    def get_paper(self, paper_id: str, fields: tuple[str, ...]) -> dict[str, Any] | None:
        safe_id = urllib.parse.quote(paper_id, safe=":")
        try:
            return self._request_json("GET", f"/paper/{safe_id}", params={"fields": ",".join(fields)})
    ...
    def get_references(self, paper_id: str, fields: tuple[str, ...]) -> list[dict[str, Any]]:
        ...
        while True:
            ...
            try:
                payload = self._request_json("GET", f"/paper/{safe_id}/references", params=params)
            except FileNotFoundError:
                return references
        ...
        for attempt in range(self.config.max_retries + 1):
            ...
            try:
                ...
                if exc.code in {429, 500, 502, 503, 504} and attempt < self.config.max_retries:
                    wait_seconds = self._retry_wait(exc, attempt)
                    LOGGER.warning("HTTP %s for %s; retrying in %.1fs", exc.code, url, wait_seconds)
                    time.sleep(wait_seconds)
                    continue
                raise RuntimeError(f"HTTP {exc.code} for {url}: {self._safe_error_body(exc)}") from exc
            except urllib.error.URLError as exc:
                if attempt < self.config.max_retries:
                    wait_seconds = self._backoff_seconds(attempt)
                    LOGGER.warning("Network error for %s: %s; retrying in %.1fs", url, exc, wait_seconds)
                    time.sleep(wait_seconds)
                    continue
    ...
    def _retry_wait(self, exc: urllib.error.HTTPError, attempt: int) -> float:
        retry_after = exc.headers.get("Retry-After") if exc.headers else None
        if retry_after:
            try:
                return max(float(retry_after), self.config.request_interval_seconds)
            except ValueError:
                pass
    ...
    def _normalize_api_key(value: str) -> str:
        ...
        try:
            ...
            raise ValueError(
                "S2_API_KEY contains non-latin1 characters. Re-copy the key and avoid smart quotes."
            ) from exc
        return normalized
